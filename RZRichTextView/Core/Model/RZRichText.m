...
- (NSAttributedString *)rz_factoryByString:(NSString *)text {
    ...
    NSAttributedString *attr = [NSAttributedString rz_colorfulConfer:^(RZColorfulConferrer *confer) {
        confer.text(text).font(weakSelf.bold? RZFontBold(weakSelf.fontSize): RZFontNormal(weakSelf.fontSize)).italic(weakSelf.italic?@0.3:@0).underLineStyle(weakSelf.underLine?1:0).underLineColor(weakSelf.textColor).strikeThrough(weakSelf.strikethrough?1:0).strikeThroughColor(weakSelf.textColor).paragraphStyle.alignment(weakSelf.aligment);
    }];
    return attr.copy;
}
