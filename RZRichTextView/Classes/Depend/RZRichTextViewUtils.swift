...
open class RZRichTextViewUtils: NSObject {
    ...
    open class func newRangeFor(origin attributedText: NSAttributedString?, range: NSRange, new attributedText1: NSAttributedString?) -> NSRange {
        ...
        // 找到一段range起点的段区域
        let firstRange = attributedText.rt.parapraghRange(for: .init(location: range.location, length: 0))
        // 找到range 结束点的段的区域
        let lastRange = attributedText.rt.parapraghRange(for: .init(location: range.rt.maxLength(), length: 0))
        // 新的所有段落range
        let newRanges = attributedText1.rt.allParapraghRange()
        // 新的一段的range
        let newFirstRange = newRanges[firstLine]
        // 结束点的range
        let newLastRange = newRanges[lastLine]
        // 得到段尾长度
        let firstEnd = firstRange.rt.maxLength() - range.location
        // 得到段尾长度
        let lastEnd = lastRange.rt.maxLength() - range.rt.maxLength()
        // 找到起点
        let star = max(newFirstRange.rt.maxLength() - firstEnd, newFirstRange.location)
        // 找到终点
        let end = max(newLastRange.rt.maxLength() - lastEnd, newLastRange.location)
...
// 正则，rt里主要用于验证是否包含制表符
public extension RZRichTextBase where T == String {
    func rangesWith(pattern: String) -> [NSRange]? {
        let string = self.rt as NSString
        let regex = try? NSRegularExpression(pattern: pattern, options: NSRegularExpression.Options.caseInsensitive)
        let matches = regex?.matches(in: self.rt, options: [], range: NSMakeRange(0, string.length))
