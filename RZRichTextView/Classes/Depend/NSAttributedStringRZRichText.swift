...
public extension NSAttributedString {
    ...
    func parapraghRange(for range: NSRange) -> NSRange {
        let string = NSString.init(string: self.string)
        var star: Int = 0
        var end: Int = 0
        var ptr: Int = 0
    ...
    func paragraphRanges(for range: NSRange) -> [NSRange] {
        ...
        let res = all.filter({ tempRang in
            if range.maxLength() < tempRang.location {
                return false
            }
            if tempRang.maxLength() <= range.location, tempRang.maxLength() != length {
                return false
            }
            return true
        })
    ...
    func paragraphAttributedSubString(form range: NSRange) -> NSAttributedString {
        ...
        return self.attributedSubstring(from: para)
    }
}
...
public extension NSAttributedString {
    ...
    func tabStyleLengthFor(_ range: NSRange) -> NSInteger {
        let rg = self.parapraghRange(for: range)
        let p = self.attributedSubstring(from: rg)
        let p1 = p.removeHeadTabString()
        return p.length - p1.length
    }
    /// 获取range所在位置的列表符类型
    func tabStyleFor(_ range: NSRange) -> RZTextTabStyle {
        let location = range.location >= self.length ? (range.location - 1) : range.location
        let t = max(location, 0)
        var enable = true
        if self.length == 0 { // nsattributedString不存在，且没有段落属性时，就不执行，否则直接调用self.attribute(.paragraphStyle, at: t, effectiveRange: nil)会crash
            enable = false
            self.enumerateAttribute(.paragraphStyle, in: .init(location: 0, length: 0), options: []) { p, _, _ in
                if let _ = p as? NSParagraphStyle {
                    enable = true
                }
            }
        }
        guard enable, let para = self.attribute(.paragraphStyle, at: t, effectiveRange: nil) as? NSParagraphStyle else { return .none}
        return para.tabStyle()
    }
    /// 文本开头是否是tab，是就返回range
    func headTabRange() -> NSRange? {
        let string = self.string
        if let range = string.rangesWith(pattern: RZTextTabStyleFormat.tabPattern)?.first, range.location == 0 {
            return range
        }
        return nil
    }
    ...
    func removeHeadTabString() -> NSAttributedString {
        if let range = self.headTabRange() {
            return self.attributedSubstring(from: .init(location: range.maxLength(), length: self.length - range.maxLength()))
        }
        return self
    }
    ...
    func unifyTabstyle() -> NSAttributedString {
        ...
        tempAttr.enumerateAttribute(.paragraphStyle, in: .init(location: 0, length: tempAttr.length), options: []) { p, range, _ in
            if let p = p as? NSParagraphStyle {
                let tab = p.tabStyle()
                let temp = p.rz_transParagraphTo(tab)
    ...
    func resetTabOrderNumber() -> NSAttributedString {
        ...
        // 得到所有的段落
        let allParas = attributedText.allParapraghRange()
        ...
        allParas.forEach { range in
            let p = attributedText.tabStyleFor(range)
        ...
        allList.forEach { formatters in
            ...
            for (i, v) in formatters.enumerated() {
                ...
                case .ul, .ol:
                    a.append(v.attributedString.removeHeadTabString())
...
public extension NSAttributedString {
    ...
    func rzrh_replaceAttributes(_ attributes: [NSAttributedString.Key: Any], range: NSRange) -> NSMutableAttributedString {
    ...
    /// 将html转换为富文本
    static func rzrh_htmlString(_ html: String?) -> NSAttributedString? {
...
public extension NSMutableAttributedString {
    /// 将富文本转换成对应的段落样式
    func rz_mutableTransParagraphTo(_ style: RZTextTabStyle) {
        var flag = false
        self.enumerateAttribute(.paragraphStyle, in: .init(location: 0, length: self.length), options: []) { p, range, _ in
            if let p = p as? NSParagraphStyle {
                flag = true
                let temp = p.rz_transParagraphTo(style)
                self.addAttribute(.paragraphStyle, value: temp, range: range)
            }
        }
        if !flag {
            let p = NSParagraphStyle.init()
            self.addAttribute(.paragraphStyle, value: p, range: .init(location: 0, length: self.length))
            self.rz_mutableTransParagraphTo(style)
        }
    }
}
